...
//  MENU
    ...
    var Nav = function(){
        ...
        if (scrollTop > NavTop) {
            $('#header .navbar-container').addClass('top');
        } else {
            $('#header .navbar-container').removeClass('top');
        }
    };
...
function ago(date) {
    ...
    if (Math.floor(seconds / (60 * 60 * 24 * 30 * 365)) >= 1) {
        return render(interval, "سال");
    }
    interval = Math.floor(seconds / (60 * 60 * 24 * 30));
    if (interval >= 1) {
        return render(interval, "ماه");
    }
    interval = Math.floor(seconds / (60 * 60 * 24));
    if (interval >= 1) {
        return render(interval, "روز");
    }
    interval = Math.floor(seconds / (60 * 60));
    if (interval >= 1) {
        return render(interval, "ساعت");
    }
    interval = Math.floor(seconds / 60);
    if (interval >= 1) {
        return render(interval, "دقیقه");
    }
    interval = Math.floor(seconds);
    return render(interval, "ثانیه");
}
